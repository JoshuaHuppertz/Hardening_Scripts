...
# Überprüfen der Berechtigungen, UID und GID der Datei /etc/group-
...
if [ -e "$l_group_file" ]; then
    ...
    if [[ "$l_stat_output" =~ Access:\ \(([^/]+)\/ ]]; then
        ...
        if [ "$l_permissions" -gt 644 ]; then
            l_output2+="\n - Datei: \"$l_group_file\" hat Berechtigung: \"$l_permissions\" (sollte 644 oder restriktiver sein)"
        else
            l_output+="\n - Datei: \"$l_group_file\" hat die erforderlichen Berechtigungen: \"$l_permissions\"."
        fi
    fi
    ...
    if [[ "$l_stat_output" =~ Uid:\ \ \(([^/]+)\/([^ ]+)\) ]]; then
        ...
        if [ "$l_uid" -ne 0 ]; then
            l_output2+="\n - Datei: \"$l_group_file\" hat UID: \"$l_uid\" (sollte 0/root sein)"
        fi
    fi
    ...
    if [[ "$l_stat_output" =~ Gid:\ \ \(([^/]+)\/([^ ]+)\) ]]; then
        ...
        if [ "$l_gid" -ne 0 ]; then
            l_output2+="\n - Datei: \"$l_group_file\" hat GID: \"$l_gid\" (sollte 0/root sein)"
        fi
    fi
else
    l_output2+="\n - Datei: \"$l_group_file\" nicht gefunden."
fi
