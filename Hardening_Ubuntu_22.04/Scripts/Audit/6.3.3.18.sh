...
# Check on-disk audit rules for /usr/sbin/usermod
if UID_MIN=$(awk '/^\s*UID_MIN/{print $2}' /etc/login.defs) && [ -n "${UID_MIN}" ]; then
    ...
    &&(/ key= *[!-~]* *$/||/ -k *[!-~]* *$/)" /etc/audit/rules.d/*.rules 2>/dev/null; then
        on_disk_output+="OK: On-disk audit rules for /usr/sbin/usermod found.\n"
    else
        on_disk_output+="Warning: On-disk audit rules for /usr/sbin/usermod not found.\n"
    fi
else
...
# Check active audit rules for /usr/sbin/usermod
if UID_MIN=$(awk '/^\s*UID_MIN/{print $2}' /etc/login.defs) && [ -n "${UID_MIN}" ]; then
    ...
    &&/ -F *path=\/usr\/sbin\/usermod/ \
    &&(/ key= *[!-~]* *$/||/ -k *[!-~]* *$/)"; then
        running_output+="OK: Running audit rules for /usr/sbin/usermod found.\n"
    else
        running_output+="Warning: Running audit rules for /usr/sbin/usermod not found.\n"
    fi
else
