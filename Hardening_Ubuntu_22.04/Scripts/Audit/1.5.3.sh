...
# Check the /etc/security/limits.conf file
check_limits_conf() {
    if grep -Ps -- '^\h*\*\h+hard\h+core\h+0\b' /etc/security/limits.conf /etc/security/limits.d/*; then
        l_output="$l_output\n - Hard core limit is set to 0 in /etc/security/limits.conf"
    else
        l_output2="$l_output2\n - Hard core limit is not set to 0 in /etc/security/limits.conf"
    fi
}
